...
QString payloadPathFromField(const QString &captured) {
    auto path = captured;
...
AiResolution aiResolutionFromJson(const QJsonObject &object) {
    AiResolution resolution;
    resolution.success = object.value(QStringLiteral("success")).toBool();
    resolution.error = object.value(QStringLiteral("error")).toString();
    resolution.errorDetails = object.value(QStringLiteral("errorDetails")).toString();
    resolution.provider = object.value(QStringLiteral("provider")).toString();
    resolution.model = object.value(QStringLiteral("model")).toString();
    ...
    for (const auto &value : object.value(QStringLiteral("findingResolutions")).toArray()) {
        const auto item = value.toObject();
        resolution.findingResolutions.append(
            {item.value(QStringLiteral("evidenceFingerprint")).toString(),
...
QStringList AiResolutionApplier::manualConflicts(const PackageRelease &project, const AiResolution &resolution) {
    ...
    for (const auto &change : resolution.changes) {
        ...
        if (payload.hasMatch()) {
            const auto path = payloadPathFromField(payload.captured(1));
...
AiApplyResult AiResolutionApplier::apply(PackageRelease &project, const AiResolution &resolution,
    ...
    PayloadReview::bindDefaultExclusions(project);
