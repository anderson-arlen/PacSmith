...
void captureAppRun(AppRunConfiguration &appRun, const QByteArray &contents) {
    ...
    if (!appRunLooksLikeScript(contents)) {
        appRun.script = false;
        appRun.reviewReason = contents.startsWith(QByteArrayView{"\x7f" "ELF", 4})
            ? QStringLiteral("Compiled ELF entry point")
            : QStringLiteral("Non-script AppRun");
        return;
    }
    appRun.script = true;
...
std::optional<SourceAnalysis> analyzeAppImage(const std::filesystem::path &path,
    ...
    for (std::filesystem::recursive_directory_iterator iterator(
        ...
        if (std::filesystem::is_directory(status)) payload.type = QStringLiteral("directory");
            ...
            payload.executable = info.permissions().testAnyFlags(
        ...
        if (payload.path == QStringLiteral("AppRun")) {
            ...
            if (payload.type == QStringLiteral("symlink")) {
                result.installMapping.appRun.script = false;
                result.installMapping.appRun.reviewReason =
            ...
            if (payload.path == QStringLiteral("AppRun")) {
                ...
                if (payload.size <= 256 * 1024) {
                    ...
                    if (file.open(QIODevice::ReadOnly)) {
                        ...
                        captureAppRun(result.installMapping.appRun, contents);
                        auto &stored = result.payload.last();
                        stored.contentSha256 = sha256Hex(contents);
                        if (result.installMapping.appRun.script) {
                            stored.textPreview = result.installMapping.appRun.contents;
                        }
                    }
                }
            }
