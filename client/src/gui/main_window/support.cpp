...
QString projectDirectory(const LibraryClient &library, const Project &project) {
    return QString::fromUtf8(library.projectsRoot().string().c_str()) + QLatin1Char('/') + project.id;
}
...
QIcon projectIcon(const LibraryClient &library, const Project &project) {
    for (const auto &release : project.releases) {
        ...
        if (release.iconArtifactId.isEmpty()) continue;
        const auto cached = library.cachedArtifactPath(release.iconArtifactId, QStringLiteral("icon"));
...
bool releaseHasRetainedPackage(const PackageRelease &release) {
    if (!release.builtArtifactIds.isEmpty()) return true;
    if (releaseHasExistingBuild(release)) return true;
    return std::any_of(release.producedPackages.cbegin(), release.producedPackages.cend(),
                       [](const auto &path) { return QFileInfo::exists(path); });
}
...
QStringList pendingPayloadReviewPaths(const PackageRelease &project, const qsizetype limit) {
    QStringList paths;
    if (project.sourceType == SourcePackageType::AppImage) return paths;
    for (const auto &entry : project.payload) {
        if (!entry.requiresReview || !PayloadReview::state(project, entry).needsReview) continue;
