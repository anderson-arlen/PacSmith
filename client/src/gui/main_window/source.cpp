...
void MainWindow::populateScripts() {
    ...
    if (scriptsActionNotice_ != nullptr) {
        if (lifecycleEditing_) {
            ...
            scriptsActionNotice_->setText(
                QStringLiteral("Review the .install script below, then approve it. That is the script pacman will run as root."));
            scriptsActionNotice_->setStyleSheet(QStringLiteral(
            ...
            scriptsActionNotice_->setText(
                lifecycle.contents.isEmpty()
                    ? QStringLiteral("No privileged Arch lifecycle script is needed.")
                    : QStringLiteral("Lifecycle script approved. Vendor scripts on the imported package tab are reference-only."));
            scriptsActionNotice_->setStyleSheet(QStringLiteral(
    ...
    if (scriptFindingsTable_ != nullptr) {
        ...
        const auto dispositions = QList<ScriptDisposition>{
            ...
            ScriptDisposition::HandledByPacSmith, ScriptDisposition::HandledByArch,
            ScriptDisposition::NotApplicable};
        for (int row = 0; row < static_cast<int>(currentRelease()->scriptFindings.size()); ++row) {
            ...
            provenanceItem->setFlags(provenanceItem->flags() & ~Qt::ItemIsEditable);
            if (finding.provenance.origin == ValueOrigin::Ai) {
                for (auto *item : {scriptItem, summaryItem, provenanceItem}) {
                    item->setBackground(QColor(35, 90, 55));
                    item->setForeground(Qt::white);
                }
            }
            const bool findingNeedsReview = findingRequiresArchAction(*currentRelease(), finding);
            if (findingNeedsReview) {
                scriptItem->setBackground(QColor(120, 92, 0));
                scriptItem->setForeground(Qt::white);
                summaryItem->setBackground(QColor(120, 92, 0));
                summaryItem->setForeground(Qt::white);
            }
            ...
            auto *handling = new QComboBox(scriptFindingsTable_);
            for (const auto disposition : dispositions) {
                handling->addItem(scriptDispositionLabel(disposition), static_cast<int>(disposition));
            }
    ...
    if (scriptsList_ != nullptr) {
        QSignalBlocker listBlock(scriptsList_);
        const auto previousRow = scriptsList_->currentRow();
        scriptsList_->clear();
        for (const auto &script : currentRelease()->maintainerScripts) {
            auto *item = new QListWidgetItem(script.name, scriptsList_);
            item->setToolTip(QStringLiteral("Imported vendor script. PacSmith never executes this file."));
        }
        if (scriptsList_->count() > 0) {
            scriptsList_->setCurrentRow(previousRow >= 0 && previousRow < scriptsList_->count()
                                            ? previousRow : 0);
        }
    }
    populating_ = false;
    if (scriptsList_ == nullptr || scriptsList_->count() == 0) {
        ...
        if (scriptStatus_ != nullptr) scriptStatus_->clear();
        if (acknowledgeScriptButton_ != nullptr) acknowledgeScriptButton_->setEnabled(false);
    } else {
        updateSelectedScript();
    }
    if (lifecycleView_ == nullptr) return;
    if (lifecycleEditing_) {
    ...
    if (lifecycle.contents.isEmpty()) {
        ...
        lifecycleStatus_->setText(QStringLiteral("%1 · blocked by validation: %2")
        ...
        lifecycleStatus_->setText(QStringLiteral("%1 · approve this exact script before installation.<br>%2")
                                      .arg(lifecycleOrigin, lifecycle.validationMessage.toHtmlEscaped()));
        acknowledgeLifecycleButton_->setEnabled(true);
        discardLifecycleButton_->setEnabled(true);
    } else {
        lifecycleStatus_->setText(QStringLiteral("%1 · exact privileged content approved<br>%2")
                                      .arg(lifecycleOrigin, lifecycle.validationMessage.toHtmlEscaped()));
        acknowledgeLifecycleButton_->setEnabled(false);
        discardLifecycleButton_->setEnabled(true);
    }
    ...
    if (unresolved == 0) {
        scriptStatus_->setText(
            script.requiresReview()
                ? QStringLiteral("Reference only. This imported script is never executed.")
                : QStringLiteral("Reference only. This exact original source was marked as read."));
    } else if (script.requiresReview()) {
        scriptStatus_->setText(
            QStringLiteral("Reference only. Configuration → Scripts maps %1 extracted responsibility item(s) onto Arch handling.")
                .arg(unresolved));
    } else {
        scriptStatus_->setText(
            QStringLiteral("Reference only. Original source acknowledged; %1 responsibility item(s) still have no Arch action.")
                .arg(unresolved));
    }
...
void MainWindow::discardLifecycleScript() {
    ...
    QString error;
    if (!library_.removeLifecycle(*project_, *currentRelease(), &error)) {
