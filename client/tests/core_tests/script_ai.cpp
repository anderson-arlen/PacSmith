...
void CoreTests::validatesLifecycleScriptsAndContentAcknowledgement() {
    ...
    pacsmith::ScriptFinding postinst;
    postinst.scriptName = QStringLiteral("postinst");
    postinst.summary = QStringLiteral("Install AppStream metadata");
    postinst.evidenceFingerprint = QString(64, QLatin1Char('a'));
    postinst.disposition = pacsmith::ScriptDisposition::LifecycleRequired;
    pacsmith::ScriptFinding prerm;
    prerm.scriptName = QStringLiteral("prerm");
    prerm.summary = QStringLiteral("Remove AppStream metadata");
    prerm.evidenceFingerprint = QString(64, QLatin1Char('b'));
    prerm.disposition = pacsmith::ScriptDisposition::LifecycleRequired;
    project.scriptFindings.append(postinst);
    project.scriptFindings.append(prerm);
    project.lifecycleScript.fileName = QStringLiteral("affine.install");
    project.lifecycleScript.contents = valid;
    project.lifecycleScript.validationPassed = true;
    project.lifecycleScript.acknowledge();
    const auto restored = pacsmith::PackageRelease::fromJson(project.toJson());
    QVERIFY(restored.lifecycleScript.sourceFingerprints.contains(postinst.evidenceFingerprint));
    QVERIFY(restored.lifecycleScript.sourceFingerprints.contains(prerm.evidenceFingerprint));
...
void CoreTests::appliesAiResolutionWithinTrustBoundaries() {
    ...
    project.scriptFindings.append(
        {QStringLiteral("prerm"), QStringLiteral("cleanup"), QStringLiteral("also needs a decision"),
         {}, QStringLiteral("second-fingerprint"),
         pacsmith::ScriptDisposition::Unresolved, {}});
    ...
    QVERIFY(!pacsmith::PayloadReview::state(project, project.payload.at(1)).needsReview);
    ...
    QCOMPARE(project.scriptFindings.first().disposition, pacsmith::ScriptDisposition::LifecycleRequired);
    QCOMPARE(project.scriptFindings.at(1).disposition, pacsmith::ScriptDisposition::LifecycleRequired);
    ...
    QVERIFY(project.lifecycleScript.sourceFingerprints.contains(QStringLiteral("second-fingerprint")));
...
void CoreTests::acknowledgesOmittedDefaultPayloadExclusions() {
    pacsmith::PackageRelease project;
    pacsmith::PayloadEntry lintian;
    lintian.path = QStringLiteral("usr/share/lintian/overrides/vendor");
    lintian.type = QStringLiteral("file");
    lintian.size = 12;
    lintian.requiresReview = true;
    lintian.reviewReason = QStringLiteral("Debian Lintian package-checker metadata has no function in an Arch package and is excluded by default");
    lintian.contentSha256 = QString(64, QLatin1Char('e'));
    project.payload.append(lintian);
    project.payloadRules.append({QStringLiteral("usr/share/lintian"), true,
                                 QStringLiteral("Debian Lintian package-checker metadata has no function in an Arch package and is excluded by default"),
                                 false, {}});
    QVERIFY(pacsmith::PayloadReview::state(project, project.payload.first()).needsReview);
    ...
    resolution.provider = QStringLiteral("chatgpt");
    resolution.model = QStringLiteral("test-model");
    resolution.changes.append({QStringLiteral("payload./usr/share/lintian/overrides/vendor.treatment"),
                               QStringLiteral("exclude"),
                               QStringLiteral("Debian-only metadata")});
    const auto applied = pacsmith::AiResolutionApplier::apply(project, resolution);
    QVERIFY2(applied.errors.isEmpty(), qPrintable(applied.errors.join(QLatin1Char('\n'))));
    const auto reviewed = pacsmith::PayloadReview::state(project, project.payload.first());
    QVERIFY(!reviewed.needsReview);
    QCOMPARE(reviewed.disposition, pacsmith::PayloadDisposition::Excluded);

    pacsmith::PackageRelease omitted = project;
    omitted.payloadRules.first().userDecision = false;
    omitted.payloadRules.first().acknowledgedFingerprint.clear();
    omitted.payloadRules.first().reason =
        QStringLiteral("Debian Lintian package-checker metadata has no function in an Arch package and is excluded by default");
    pacsmith::AiResolution empty;
    empty.success = true;
    empty.provider = QStringLiteral("chatgpt");
    empty.model = QStringLiteral("test-model");
    const auto bound = pacsmith::AiResolutionApplier::apply(omitted, empty);
    Q_UNUSED(bound);
    QVERIFY(!pacsmith::PayloadReview::state(omitted, omitted.payload.first()).needsReview);
    QCOMPARE(pacsmith::PayloadReview::state(omitted, omitted.payload.first()).disposition,
             pacsmith::PayloadDisposition::ExcludedByDefault);
}
