...
void CoreTests::tracksContentSpecificPayloadDecisions() {
    ...
    QVERIFY(defaultState.needsReview);
    pacsmith::PayloadReview::bindDefaultExclusions(aptProject);
    const auto boundDefault = pacsmith::PayloadReview::state(aptProject, aptProject.payload.first());
    QVERIFY(!boundDefault.needsReview);
    QCOMPARE(boundDefault.disposition, pacsmith::PayloadDisposition::ExcludedByDefault);
    QCOMPARE(pacsmith::PayloadEntry::fromJson(
                 QJsonObject{{QStringLiteral("path"), QStringLiteral("etc/foo")},
                             {QStringLiteral("type"), QStringLiteral("file")},
                             {QStringLiteral("size"), 42}}).size,
             42);
    ...
    pacsmith::PackageRelease sandboxProject;
    pacsmith::PayloadEntry sandbox;
    sandbox.path = QStringLiteral("usr/lib/app/chrome-sandbox");
    sandbox.type = QStringLiteral("file");
    sandbox.size = 15 * 1024 * 1024;
    sandbox.requiresReview = true;
    sandbox.executable = true;
    sandboxProject.payload.append(sandbox);
    pacsmith::PayloadReview::decide(sandboxProject, sandbox.path, false);
    QVERIFY(!pacsmith::PayloadReview::state(sandboxProject, sandboxProject.payload.first()).needsReview);
    sandboxProject.payload.first().contentSha256 = QString(64, QLatin1Char('d'));
    QVERIFY(pacsmith::PayloadReview::state(sandboxProject, sandboxProject.payload.first()).needsReview);
    pacsmith::PayloadReview::adoptFilledContentHash(sandboxProject, sandbox.path);
    const auto hashedKeep = pacsmith::PayloadReview::state(sandboxProject, sandboxProject.payload.first());
    QVERIFY(!hashedKeep.needsReview);
    QCOMPARE(hashedKeep.disposition, pacsmith::PayloadDisposition::Included);
    sandboxProject.payload.first().contentSha256 = QString(64, QLatin1Char('e'));
    pacsmith::PayloadReview::adoptFilledContentHash(sandboxProject, sandbox.path);
    QVERIFY(pacsmith::PayloadReview::state(sandboxProject, sandboxProject.payload.first()).needsReview);
}
