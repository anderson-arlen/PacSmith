...
Vendors that publish behind an interactive download page or have no machine-readable release channel can use Manual monitoring. On the Versions tab, **Submit New Release** accepts a locally downloaded vendor artifact, an explicit version when the artifact does not identify itself, and an optional publisher SHA-256. The artifact then follows the same verification, inspection, configuration carry-forward, review, build, and repository workflow as an automatically discovered release.
...
The running GUI watches its client settings file and subscribes to authenticated daemon change events. Projects, releases, jobs, repository state, library policy, credentials, listeners, enrollments, and clients changed through MCP or another remote client update live. Harness profiles and tray/login preferences continue to update through the local settings watcher. Unsaved GUI drafts are preserved and must be explicitly reloaded before they can be replaced or saved.
