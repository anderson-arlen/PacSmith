...
QString iconSourceName(const PackageRelease &project) {
    ...
    if (icon.missing || icon.projectPath.isEmpty() || icon.sha256.isEmpty()) return {};
...
void appendSelectedIcon(QString &result, const PackageRelease &project) {
    ...
    const auto extension = icon.format.isEmpty()
        ? QFileInfo(icon.projectPath).suffix().toLower() : icon.format.toLower();
    const auto sourceName = QStringLiteral("${_PACSMITH_ICON}");
    ...
    result += QStringLiteral("  install -Dm644 \"$srcdir/%1\" \"$pkgdir%2/%3.%4\"\n")
                  .arg(sourceName, doubleQuotedPath(directory),
...
QString PkgbuildGenerator::identityVariables(const PackageRelease &project) {
    ...
    appendAssignment(result, QStringLiteral("_PACSMITH_URL"), project.debian.homepage);
