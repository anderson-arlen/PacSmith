...
MCP project creation accepts the same remote sources as the GUI. GitHub repository, release, and exact asset URLs flow through `GitHubUpdateService`, then PacSmith's artifact downloader and `LibraryClient::importSource`; generic first-party HTTPS artifacts use the same download/import boundary through `import_direct_url`. `import_artifact` also targets an existing project for a human-supplied Manual release, with an explicit version override and optional publisher SHA-256 enforced by `pacsmithd`. Temporary bytes remain PacSmith-owned, exact GitHub asset links become exact-match selectors automatically, and ambiguous links report candidates instead of encouraging an agent-side curl workflow.
