...
MCP project creation accepts the same remote sources as the GUI. GitHub repository, release, and exact asset URLs flow through `GitHubUpdateService`, then PacSmith's artifact downloader and `LibraryClient::importSource`; generic first-party HTTPS artifacts use the same download/import boundary through `import_direct_url`. For a Manual release, `import_artifact` accepts a human-supplied local file while `import_direct_url` keeps temporary bytes PacSmith-owned; both target an existing project and accept an explicit version override and optional publisher SHA-256 enforced by `pacsmithd`. The URL downloader rejects a checksum mismatch before upload as well. Exact GitHub asset links become exact-match selectors automatically, and ambiguous links report candidates instead of encouraging an agent-side curl workflow.
...
Update discovery and preparation remain deterministic. Each project then chooses `never`, `review_free`, or `ai` automatic-build handling. `review_free` requires a previously successful, fully reviewed package configuration; unchanged vendor dependency declarations and lifecycle behavior; no current structured review issues; available Arch dependencies; and an enabled, signed repository with publication enabled for the project. It is unavailable for Custom PKGBUILDs because arbitrary recipe compatibility cannot be inferred. The `ai` policy launches the existing default external harness as a visible, fire-and-forget session when deterministic review blocks progress or the candidate uses a Custom PKGBUILD. The scoped prompt directs the agent through MCP, allows at most one obvious and simple follow-up repair, and otherwise leaves the session for human attention. The checker records a successful dispatch on the release so later checks do not launch duplicates. A successful build is marked as automatic and published to unstable. PacSmith never installs an update automatically.
