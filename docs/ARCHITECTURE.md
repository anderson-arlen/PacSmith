...
`BuildService` owns an asynchronous `QProcess`, sets the project as its working directory, invokes `makepkg` with no shell, captures both output streams and timestamps, and scans for produced `.pkg.tar.*` files. The CLI drives it with a `QCoreApplication` event loop; the GUI consumes the same signals without blocking its event loop.
